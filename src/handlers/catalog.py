...
async def category(callback: CallbackQuery, db: MDB):
    ...
    await callback.message.edit_media(
        InputMediaPhoto(
            ...
            caption="Вам доступны следующие товары ⬇️"
        ),
...
async def product(callback: CallbackQuery, db: MDB):
    product = await db.products.find_one(dict(_id=int(callback.data.split("_")[1])))
    category = await db.categories.find_one(dict(_id=product["category_id"]))
    ...
    await callback.message.edit_caption(
        ...
        reply_markup=inline_builder(
            text=["💳 Купить", "В главное меню ⬅️"],
            callback_data=[f"buy_{product['_id']}", "to_main_menu"]
        )
    )
    await callback.answer()
...
async def buy(callback: CallbackQuery, db: MDB):
    await callback.answer("На данный момент оплата не работает.")